...
using XLHFramework.GCFrameWork.Runtime;
...
namespace XLHFramework.GCFrameWork.World
{
    public class WorldManager
    {
        ...
        public static void CreateWorldByReflection(Assembly assembly, string worldFullName = "XGC.SK.SKWorld")
        {
        ...
        {
            Assembly asm = typeof(IBehaviourExecution).Assembly;
            Type[] types = asm.GetTypes().Where(t => t.Namespace == "XLHFramework.GCFrameWork.Runtime").ToArray();
            Type excutionType = typeof(IBehaviourExecution);
         
            foreach (var type in types)
            {
                if (excutionType.IsAssignableFrom(type))
                {
                    FieldInfo worldNameField = type.GetField("worldName", BindingFlags.Static | BindingFlags.Public);
                    FieldInfo[] fields = type.GetFields(BindingFlags.Public | BindingFlags.Static);
                    string worldName = worldNameField.GetValue(null) as string;
                    if (world.GetType().Name == worldName)
                    {
                        CurWorldEnum = (WorldEnum)Enum.Parse(typeof(WorldEnum), worldName);
                        return Activator.CreateInstance(type)  as IBehaviourExecution;
                    }
                }
            }
            ...
            /*if (world.GetType().Name == "HallWorld")
        {
            ...
            return new HallWorldScriptExecutionOrder();#1#
        }
        else if (world.GetType().Name == "BottleWorld")
        {
         //   CurWorldEnum = WorldEnum.BottleWorld;
            //return new HallWorldScriptExecutionOrder();
        }
        else if (world.GetType().Name == "SoldierWorld")
        {
           // CurWorldEnum = WorldEnum.SoldierWorld;
           // return new SoldierWorldScriptExecutionOrder();
        }
        else if(world.GetType().Name == "FlyDiscWorld")
        {
           // CurWorldEnum = WorldEnum.SoldierWorld;
        }
        else if(world.GetType().Name == "ZomWorld")
        {
          //  CurWorldEnum = WorldEnum.ZomWorld;
        }
        else if (world.GetType().Name == "MouseWorld")
        {
           // CurWorldEnum = WorldEnum.MouseWorld;
        }*/
