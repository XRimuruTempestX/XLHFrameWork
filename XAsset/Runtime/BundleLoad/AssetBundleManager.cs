...
namespace XAsset.Runtime.BundleLoad
{
    ...
    public class AssetBundleManager : Singleton<AssetBundleManager>
    {
        ...
        /// 正在加载中的Unitask
        /// </summary>
        private Dictionary<string, UniTaskCompletionSource<AssetBundle>> mLoadUniTaskDic =
            new Dictionary<string, UniTaskCompletionSource<AssetBundle>>();

        /// <summary>
        /// 缓存池
        /// </summary>
        private ClassObjectPool<AssetBundleCache> assetbundleCachePool =  new ClassObjectPool<AssetBundleCache>(100);
        ...
        public async UniTask<bool> InitAssetModule(string bundleModule)
        {
            ...
            try
            {
                ...
                if (GeneratorBundleConfigPath(bundleModule))
                {
                    ...
                    LogError("bundleConfigJson is null !!!  ----->" + mAssetsBundleConfigName);
        ...
        /// <summary>
        /// 生成模块AssetBundleConfig配置文件路径
        /// </summary>
        /// <param name="bundleModule"></param>
        /// <returns></returns>
        private bool GeneratorBundleConfigPath(string bundleModule)
        {
        ...
        /// <returns></returns>
        public async UniTask<AssetBundle> LoadAssetBundle(string bundleName, string bundleModuleType)
        {
            ...
            if (bundle == null || (bundle != null && bundle.assetBundle == null))
            {
                try
                {
                    bundle = assetbundleCachePool.Spawn();
                    //计算AssetBundle加载路径
                    ...
                    else
                    {
                        ...
                        {
                            tcs =  new UniTaskCompletionSource<AssetBundle>();
                            var assetUniTask = AssetBundle.LoadFromFileAsync(bundlePath)
                                .ToUniTask(cancellationToken: _cancellationTokenSource.Token);
                            mLoadUniTaskDic.TryAdd(bundleName, tcs);
                            Log("Created new load task for bundle=" + bundleName + " (file) path=" + bundlePath);
                            bundle.assetBundle = await assetUniTask;
                            tcs.TrySetResult(bundle.assetBundle);
                            mLoadUniTaskDic.Remove(bundleName);
                        }
